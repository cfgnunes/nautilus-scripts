...
_main() {
    ...
    #script_test="Encryption/GPG: Encrypt with keys"
    #__echo_script "$script_test"
    #bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    #__test_file_nonempty "$output_file"
    #__test_file_empty "$std_output"

    #script_test="Encryption/GPG: Encrypt with keys (ASCII)"
    #__echo_script "$script_test"
    #bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    #__test_file_nonempty "$output_file"
    #__test_file_empty "$std_output"
    ...
    script_test="Document/PDF: Encrypt, Decrypt/PDF: Find password-encrypted PDFs"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$temp_dir" >"$std_output"
    __test_file_empty "$std_output"
    ...
    script_test="Plain text/Text: Encoding/Text: Remove accents"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$output_file (ASCII).txt"
    __test_file_empty "$std_output"

    script_test="Plain text/Text: Encoding/Text: List encoding"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$std_output"
