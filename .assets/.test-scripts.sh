...
_main() {
    ...
    script_test="Document/PDF: Page size/PDF: Page size to A3"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$output_file (paper A3).pdf"
    __test_file_empty "$std_output"

    script_test="Document/PDF: Page size/PDF: Page size to A4"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$output_file (paper A4).pdf"
    __test_file_empty "$std_output"

    script_test="Document/PDF: Page size/PDF: Page size to A5"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$output_file (paper A5).pdf"
    __test_file_empty "$std_output"

    script_test="Document/PDF: Page size/PDF: Page size to US Legal"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
    __test_file_nonempty "$output_file (paper US Legal).pdf"
    __test_file_empty "$std_output"

    script_test="Document/PDF: Page size/PDF: Page size to US Letter"
    __echo_script "$script_test"
    bash "$ROOT_DIR/$script_test" "$input_file1" >"$std_output"
